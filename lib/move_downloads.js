...
module.exports = function(copy, callback) {
	...
	async.each(copy, function(episode, next) {
		...
		if(config.detailed_information) console.log(episode)
		...
		if(config.move) {
			fse.move(episode.copy, episode.destination, function(err) {
				...
				// @todo Now that it's either moved or already exists remove the file / folder from download directory
				remove_download(episode, function() { })

			}) // fse.copy
		}
		else {
			console.log(chalk.yellow('Move disabled in config'))
		}
		next(null)
